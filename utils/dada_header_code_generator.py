...
for key in dada_header:
    ...
    source_file.write(
        f'  if (ascii_header_get(dada_header_buffer, "{key}", {data_type_marker}, &dada_header->{key.lower()}) < 0)'
    )
    source_file.write("  {\n")
    source_file.write(
...
for key in dada_header:
    ...
    source_file.write(
        f'  if (ascii_header_set(dada_header_buffer, "{key}", {data_type_marker}, dada_header.{key.lower()}) < 0)'
    )
    source_file.write("  {\n")
    source_file.write(
